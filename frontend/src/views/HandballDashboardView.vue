    ...
    <main class="dashboard-main">
      <div class="container">
        <section class="welcome-section fade-in">
          <h1>Czech Handball Extraliga</h1>
          <p>Simulate the Czech handball league with real teams, playoffs, and relegation battles</p>
        </section>
        ...
        <section class="worlds-section">
          ...
          <div v-else-if="worlds.length === 0" class="empty-state fade-in">
            ...
            <p>Create your first Czech Handball Extraliga simulation to get started!</p>
            <button @click="showCreateModal = true" class="btn btn-primary">
              <i class="fa-solid fa-plus"></i>
          ...
          <div v-else class="worlds-grid">
            ...
            >
              ...
              <h3>{{ world.name }}</h3>
              <p class="world-description">{{ world.description || 'Czech Extraliga simulation' }}</p>
      ...
      <div class="modal fade-in">
        ...
        <form @submit.prevent="handleCreateWorld">
          <div class="form-group">
            <label class="form-label" for="worldName">League Name</label>
            <input
              id="worldName"
              v-model="newWorld.name"
              type="text"
              class="input-field"
              placeholder="e.g., My Extraliga 2024"
              required
            />
          </div>
          <div class="form-group">
...
const worlds = computed(() => handballStore.worlds)
...
function enterWorld(worldId) {
  router.push(`/handball/league/${worldId}`)
}
...
async function handleCreateWorld() {
  ...
  try {
    const world = await handballStore.createWorld(newWorld.value.name, newWorld.value.description)
    showCreateModal.value = false
    newWorld.value = { name: '', description: '' }
    router.push(`/handball/league/${world.id}`)
  } catch (err) {
    createError.value = err.response?.data?.error || 'Failed to create league'
  } finally {
...
async function handleDeleteWorld() {
  ...
  deleting.value = true
  try {
    await handballStore.deleteWorld(worldToDelete.value.id)
...
onMounted(async () => {
  try {
    await handballStore.fetchWorlds()
  } catch (error) {
