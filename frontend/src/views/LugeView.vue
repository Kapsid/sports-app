      ...
      <div class="container header-content">
        ...
        <div class="breadcrumb" v-if="world">
          ...
          <span class="sport-name">
            <i class="fa-solid fa-person-sledding"></i>
            Luge
          </span>
        </div>
        <div class="user-menu">
      ...
      <div class="container">
        ...
        <div v-if="activeTab === 'athletes'" class="tab-content fade-in">
          ...
          <div v-else-if="athletes.length === 0" class="empty-state">
            ...
            <p>Create your first luge athlete to get started.</p>
        ...
        <div v-if="activeTab === 'season'" class="tab-content fade-in">
          ...
          <div v-else-if="!currentSeason" class="empty-state">
            ...
            <p>Start a new Luge World Cup season to begin competing.</p>
          ...
          <div v-else class="season-content">
            <div class="season-header-card">
              <div class="season-info">
                <h2><i class="fa-solid fa-trophy"></i> Luge World Cup {{ currentSeason.name }}</h2>
          ...
          <div class="standings-compact">
            ...
            <div v-if="standingsView === 'teams'" class="standings-panel teams-panel">
              <div v-if="teamStandings.length === 0" class="no-standings">
                <i class="fa-solid fa-people-group"></i>
                <p>No team standings yet. Assign athletes to teams and complete races!</p>
              </div>
              <div v-else class="standings-list-compact">
                <div
                ...
                >
                  ...
                  <div class="standing-stats">
                    <span class="standing-points team-pts">{{ team.points }} pts</span>
      ...
      <div class="modal race-modal fade-in">
        ...
        <div class="race-modal-content">
          ...
          <div class="animation-section">
            <!-- Track SVG - Luge style (compact) -->
            <div class="track-container">
              <svg viewBox="0 0 400 140" class="track-svg">
                <defs>
                  <linearGradient id="lugeIceGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                <rect x="0" y="0" width="400" height="140" fill="url(#lugeIceGradient)" />
                <!-- Compact track path -->
                ...
                <!-- Animated Luge Sled -->
                <g class="luge-sled" :transform="`translate(${sledPosition.x * 0.8}, ${sledPosition.y * 0.47}) rotate(${sledRotation})`">
                  <rect x="-8" y="-2" width="16" height="4" rx="2" fill="#ea580c" filter="url(#lugeGlow)" />
                  <ellipse cx="2" cy="0" rx="4" ry="2" fill="#f97316" />
                  <circle cx="5" cy="-0.5" r="2" fill="#fed7aa" />
                  <rect v-if="animating" x="-14" y="-1" width="6" height="2" rx="1" fill="#ea580c" opacity="0.4" />
                </g>
              </svg>
            </div>
...
import { useLugeStore } from '../stores/luge'
...
// Animation
const sledPosition = ref({ x: 25, y: 35 })
...
// Computed
const athletes = computed(() => lugeStore.athletes)
const currentSeason = computed(() => lugeStore.currentSeason)
const seasonEvents = computed(() => lugeStore.events)
const seasonStandings = computed(() => lugeStore.standings)
const countryNames = computed(() => lugeStore.countryNames)
...
async function loadAthletes() {
  ...
  try {
    await lugeStore.fetchAthletes(worldId.value)
  } catch (error) {
...
async function loadSeason() {
  ...
  try {
    await lugeStore.fetchCurrentSeason(worldId.value)
  } catch (error) {
...
async function loadTeamStandings() {
  try {
    await teamsStore.fetchTeamStandings(worldId.value, 'luge')
  } catch (error) {
...
async function handleCreateSeason() {
  ...
  try {
    await lugeStore.createSeason(worldId.value)
  } catch (error) {
...
async function handleResetSeason() {
  try {
    await lugeStore.resetSeason(currentSeason.value.id)
...
async function handleSaveAthlete() {
  ...
  try {
    if (editingAthlete.value) {
      await lugeStore.updateAthlete(editingAthlete.value.id, athleteForm.value)
    } else {
      await lugeStore.createAthlete(worldId.value, athleteForm.value)
    }
    closeAthleteModal()
  } catch (error) {
...
async function handleDeleteAthlete() {
  try {
    await lugeStore.deleteAthlete(athleteToDelete.value.id)
...
async function handleDeleteAllAthletes() {
  try {
    await lugeStore.deleteAllAthletes(worldId.value)
...
async function simulateAthleteRun(athlete) {
  ...
  // Check for DNF
  const dnfChance = 0.015 * (1 + (100 - consistency) / 100 * 0.25 + (100 - skillSteering) / 100 * 0.2)
  const isDNF = Math.random() < dnfChance

  // Calculate time
  let time = baseTime
  time += (70 - skillStart) * 0.012
  time += (70 - skillSteering) * 0.02
  time += (70 - skillLines) * 0.015
  time += (70 - form) * 0.008
  time += (Math.random() - 0.5) * 0.4 * (1.5 - consistency / 100)
...
async function finishRun() {
  ...
  try {
    if (currentRun.value === 1) {
      await lugeStore.simulateRace(selectedEvent.value.id, results)
    } else if (currentRun.value === 2) {
      await lugeStore.simulateRun2(selectedEvent.value.id, results)
    } else if (currentRun.value === 3) {
      await lugeStore.simulateRun3(selectedEvent.value.id, results)
    } else if (currentRun.value === 4) {
      await lugeStore.simulateRun4(selectedEvent.value.id, results)
    }
...
.results-table-header.four-run {
  grid-template-columns: 50px 1fr 70px 70px 70px 70px 90px 70px;
}
...
.col-pos.pos-gold { color: #f59e0b; }
.col-pos.pos-silver { color: #6b7280; }
.col-pos.pos-bronze { color: #d97706; }
...
.skill-elite { color: #16a34a; font-weight: 700; }
.skill-high { color: #2563eb; font-weight: 600; }
.skill-good { color: #475569; }
.skill-average { color: #d97706; }
.skill-low { color: #dc2626; }
