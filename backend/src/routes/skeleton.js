...
const { generateSkeletonCalendar, worldCupPoints, skeletonCountryNames } = require('../data/skeletonCalendar');
...
// Simulate a single run for a skeleton athlete
function simulateRun(athlete, trackLength, isRun2 = false) {
  ...
  // Base time depends on track length (skeleton similar speed to luge)
  const baseTime = trackLength / 26;

  // Check for DNF (crash) - skeleton has slightly higher crash rate (head first)
  const baseDNFRate = 0.018;
  const dnfModifier = (100 - consistency) / 100 * 0.28 + (100 - skillSteering) / 100 * 0.22;
  ...
  // Start effect (running start with sled)
  const startEffect = (70 - skillStart) * 0.014;

  // Steering effect (body position steering)
  const steeringEffect = (70 - skillSteering) * 0.018;

  // Lines effect (choosing optimal racing lines)
  const linesEffect = (70 - skillLines) * 0.016;

  // Form effect (daily condition)
  const formEffect = (70 - form) * 0.009;

  // Random variation based on consistency
  const consistencyMultiplier = 1.5 - (consistency / 100);
  const randomVariation = randomNormal(0, 0.22 * consistencyMultiplier);

  // Day factor (ice conditions, equipment, etc.)
  const dayFactor = randomNormal(0, 0.11);
  ...
  for (let i = 0; i < trackSections; i++) {
    sectionVariation += randomNormal(0, 0.007);
  }

  // Run 2+ has slightly different ice conditions
  const run2Modifier = isRun2 ? randomNormal(0.07, 0.14) : 0;
  ...
  const mistakeChance = (100 - consistency) / 100 * 0.18;
  if (Math.random() < mistakeChance) {
    time += randomNormal(0.12, 0.06);
  }
...
// Format time as SS.ccc (milliseconds precision)
...
// Get all skeleton athletes for a world
router.get('/world/:worldId/athletes', authMiddleware, (req, res) => {
  try {
    ...
    const athletes = all('SELECT * FROM skeleton_athletes WHERE world_id = ? ORDER BY last_name, first_name', [worldId]);

    res.json({ athletes, countryNames: allCountries });
  } catch (error) {
    console.error('Error fetching skeleton athletes:', error);
    res.status(500).json({ error: 'Failed to fetch athletes' });
  }
});

// Create a new skeleton athlete
router.post('/world/:worldId/athlete', authMiddleware, (req, res) => {
  try {
    ...
    const id = uuidv4();
    run(`
      INSERT INTO skeleton_athletes (id, world_id, first_name, last_name, country, skill_start, skill_steering, skill_lines, consistency, form)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `, [id, worldId, first_name, last_name, country,
        skill_start || 70, skill_steering || 70, skill_lines || 70, consistency || 70, form || 70]);

    const athlete = get('SELECT * FROM skeleton_athletes WHERE id = ?', [id]);
    res.json({ athlete, countryNames: allCountries });
  } catch (error) {
    console.error('Error creating skeleton athlete:', error);
    res.status(500).json({ error: 'Failed to create athlete' });
  }
});

// Update a skeleton athlete
router.put('/athlete/:id', authMiddleware, (req, res) => {
  try {
    ...
    const athlete = get(`
      SELECT a.* FROM skeleton_athletes a
    ...
    const updatedAthlete = get('SELECT * FROM skeleton_athletes WHERE id = ?', [id]);
    res.json({ athlete: updatedAthlete, countryNames: allCountries });
  } catch (error) {
    console.error('Error updating skeleton athlete:', error);
    res.status(500).json({ error: 'Failed to update athlete' });
  }
});

// Delete a skeleton athlete
router.delete('/athlete/:id', authMiddleware, (req, res) => {
  try {
    ...
    run('DELETE FROM skeleton_athletes WHERE id = ?', [id]);
    res.json({ message: 'Athlete deleted' });
  } catch (error) {
    console.error('Error deleting skeleton athlete:', error);
    res.status(500).json({ error: 'Failed to delete athlete' });
  }
});

// Delete all skeleton athletes for a world
router.delete('/world/:worldId/all-athletes', authMiddleware, (req, res) => {
  try {
    ...
    run('DELETE FROM skeleton_athletes WHERE world_id = ?', [worldId]);
    res.json({ message: 'All athletes deleted' });
  } catch (error) {
    console.error('Error deleting all skeleton athletes:', error);
...
// Get current skeleton season for a world
router.get('/world/:worldId/current-season', authMiddleware, (req, res) => {
  try {
    ...
    const events = all(
      'SELECT * FROM skeleton_events WHERE season_id = ? ORDER BY event_index',
      [season.id]
    );
    ...
    console.error('Error fetching skeleton season:', error);
    res.status(500).json({ error: 'Failed to fetch season' });
  }
});

// Create new skeleton season
router.post('/world/:worldId/create-season', authMiddleware, (req, res) => {
  try {
    ...
    run(`
      INSERT INTO skeleton_seasons (id, world_id, name, year_start, year_end, status)
      VALUES (?, ?, ?, ?, ?, 'not_started')
    ...
    for (const event of calendar) {
      ...
      run(`
        INSERT INTO skeleton_events (id, season_id, event_index, name, location, country, track_length, date, status, runs, championship)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'scheduled', ?, ?)
    ...
    const season = get('SELECT * FROM skeleton_seasons WHERE id = ?', [seasonId]);
    const events = all('SELECT * FROM skeleton_events WHERE season_id = ? ORDER BY event_index', [seasonId]);
    ...
    console.error('Error creating skeleton season:', error);
...
router.get('/event/:eventId', authMiddleware, (req, res) => {
  try {
    ...
    const event = get(`
      SELECT e.*, s.world_id
      FROM skeleton_events e
      INNER JOIN skeleton_seasons s ON e.season_id = s.id
    ...
    const athletes = all('SELECT * FROM skeleton_athletes WHERE world_id = ? ORDER BY last_name, first_name', [event.world_id]);
    ...
    console.error('Error fetching skeleton event:', error);
    res.status(500).json({ error: 'Failed to fetch event' });
  }
});

// Save Run 1 results
router.post('/event/:eventId/simulate', authMiddleware, validateWeekLock('skeleton_events', 'skeleton_seasons'), (req, res) => {
  try {
    const { eventId } = req.params;
    ...
    if (frontendResults && frontendResults.length > 0) {
      ...
      const athletes = all('SELECT * FROM skeleton_athletes WHERE world_id = ?', [event.world_id]);
    ...
    run('UPDATE skeleton_events SET status = ?, run1_results = ? WHERE id = ?',
    ...
    console.error('Error simulating skeleton run 1:', error);
...
router.post('/event/:eventId/simulate-run2', authMiddleware, (req, res) => {
  try {
    ...
    if (isFourRunEvent) {
      run('UPDATE skeleton_events SET status = ?, run2_results = ? WHERE id = ?',
      ...
      run('UPDATE skeleton_events SET status = ?, run2_results = ?, results = ? WHERE id = ?',
    ...
    console.error('Error simulating skeleton run 2:', error);
...
router.post('/event/:eventId/simulate-run3', authMiddleware, (req, res) => {
  try {
    ...
    run('UPDATE skeleton_events SET status = ?, run3_results = ? WHERE id = ?',
    ...
    console.error('Error simulating skeleton run 3:', error);
...
router.post('/event/:eventId/simulate-run4', authMiddleware, (req, res) => {
  try {
    ...
    run('UPDATE skeleton_events SET status = ?, run4_results = ?, results = ? WHERE id = ?',
    ...
    console.error('Error simulating skeleton run 4:', error);
...
function updateStandingsAfterRace(seasonId, results) {
  ...
  run('UPDATE skeleton_seasons SET standings = ?, status = ? WHERE id = ?',
...
router.post('/season/:seasonId/reset', authMiddleware, (req, res) => {
  try {
    ...
    run(`
      UPDATE skeleton_seasons
      SET standings = '[]', status = 'not_started'
      WHERE id = ?
    `, [seasonId]);

    run('UPDATE skeleton_events SET status = ?, results = ?, run1_results = ?, run2_results = ?, run3_results = ?, run4_results = ? WHERE season_id = ?',
      ['scheduled', '[]', '[]', '[]', '[]', '[]', seasonId]);

    const updatedSeason = get('SELECT * FROM skeleton_seasons WHERE id = ?', [seasonId]);
    const events = all('SELECT * FROM skeleton_events WHERE season_id = ? ORDER BY event_index', [seasonId]);
    ...
    console.error('Error resetting skeleton season:', error);
